...
import InterfazGrafica as InterfazGrafica
import json
...
def posponerPartida(tab_Ejecucuon,lista_total_persona,lista_total_maquina,AB,jugador1,maquina,tiempo_actual,tablero,nivel):
    ...
    archivo1 = open ('Almacenamiento/bolsa.json','r')
    ...
    archivo = open ('Almacenamiento/posponerPartida.json','w')
    json.dump(lista_posponer,archivo)
    archivo.close()
    sg.theme('black')
    layout = [
        [sg.Image(r'Imagenes/POSPONER1.png')],
...
def Top10():
    ...
    archivo = open('Almacenamiento/TopJugadores.json','r')
...
def FinDelJuego(window,maquina,Jugador1,Top,listas_palabras,op,tab,no_hay_partida):
    ...
    if Jugador1.get_puntaje_total()>maquina.get_puntaje_total() or maquina.get_cant_paso()==3:
        sg.theme('black')
        layout = [
            [sg.Image(r'Imagenes/ganaste.png')],  #Imagen hecha por los integrantes   fin_del_juego.png
            [sg.Button("¡Entendido!",key="ok")]
         ]
        ...
        sg.theme('black')
        layout = [
            [sg.Image(r'Imagenes/perdiste.png')],  #Imagen hecha por los integrantes   fin_del_juego.png
            [sg.Button("¡Entendido!",key="ok")]
         ]
    ...
    if listas_palabras != "" and op==False:
        sg.theme('black')
        layout = [
            [sg.Image(r'Imagenes/fin_del_juego.png')],  #Imagen hecha por los integrantes   fin_del_juego.png
            [sg.Button("¡Entendido!",key="ok")]
         ]
        ...
        sg.theme('black')
        layout = [
            [sg.Image(r'Imagenes/tiempo.png')],          #Imagen hecha por los integrantes
            [sg.Button("¡Entendido!",key="ok")]
         ]
